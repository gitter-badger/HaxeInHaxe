		...
		try {
			...
			function local(v) return {
				try {
					...
					var v' = alloc_var(v.v_name, v.v_type);
				...
				var i = { () with i_var = v;
						  i_subst = v';
									...
									case TConst(TSuper): { (ethis) with eexpr = TConst(TThis) };
									case _: ethis;
								};
									...
									case TReturn(eo) if(!(in_local_fun.val)): if (!(term)) {
									...
									case TTry(e1, catches): { (e) with eexpr = TTry(map(term, e1), List.map(function (v, e): var lv = local(v).i_subst;
									var e = map(term, e);
									...
									case TSwitch(cond, cases, Some(def)): ||(has_term_return(cond), List.for_all(has_term_return, ::(def, List.map(snd, cases))));
									...
									::(e, []);
									case ::({ eexpr = TIf(cond, e1, None) } = e, l) if(&&(term, has_term_return(e1))): loop(::({ (e) with eexpr = TIf(cond, e1, Some(mk(TBlock(l), e.etype, e.epos)));
									...
									case _: { (e) with eexpr = TObjectDecl(fl) };
								};
									...
									if (||(cancel_inlining.val, &&(!(Common.defined(ctx.com, Define.Analyzer)), &&(Common.platform(ctx.com, Js), &&(!(force.val), ||(<>(init, None), has_vars.val)))))) {
									None;
								} else {
									...
									try {
									switch (follow(e.etype)) {
									case TMono(_) | TInst({ cl_kind = KTypeParameter(_) }, _): switch (follow(tret)) {
									...
									case (_, None) if(!(has_return_value.val)): { (e) with etype = tret };
									...
									case TAnon(a): switch (a.a_status.val) {
									case Statics({ cl_kind = KAbstractImpl(a) }) if(Meta.has(Meta.Impl, cf.cf_meta)): if (<>(cf.cf_name, "_new")) {
									var tb = TFun(::((new Tuple("", False, map_type(a.a_this))), List.map(function e: (new Tuple("", False, e.etype)), List.tl(params))), tret);
									...
									case (TNew({ cl_path = ([], IntIterator) }, [], ::(i1, ::(i2, []))), _): var max = switch ((new Tuple(i1.eexpr, i2.eexpr))) {
									...
									switch (e.eexpr) {
									case TBinop(OpAssign, { eexpr = TLocal(l) }, _) | TBinop(OpAssignOp(_), { eexpr = TLocal(l) }, _) | TUnop(Increment, _, { eexpr = TLocal(l) }) | TUnop(Decrement, _, { eexpr = TLocal(l) }) if(==(l, i)): error("Loop variable cannot be modified", e.epos);
									...
									case _: { (i2) with eexpr = TCast(i2, None);
									etype = t_int };
								};
									switch (max) {
									case None: lblock(::(mk(TVar(tmp, Some(i1)), t_void, p), ::(mk(TWhile(mk(TBinop(OpLt, etmp, i2), ctx.t.tbool, p), block, NormalWhile), t_void, p), [])));
									case Some(max): lblock(::(mk(TVar(tmp, Some(i1)), t_void, p), ::(mk(TVar(max, Some(i2)), t_void, p), ::(mk(TWhile(mk(TBinop(OpLt, etmp, mk(TLocal(max), t_int, p)), ctx.t.tbool, p), block, NormalWhile), t_void, p), []))));
								};
									case (TArrayDecl(el), TInst({ cl_path = ([], Array) }, ::(pt, []))) if(False): try {
									var num_expr = ref(0);
									function loop(e) return {
									switch (fst(e)) {
									...
									case (_, TInst({ cl_path = ([], Array) }, ::(pt, []))) | (_, TInst({ cl_path = (::(flash, []), Vector) }, ::(pt, []))): gen_int_iter(pt, get_next_array_element, get_array_length);
									...
									False;
								}, !(PMap.mem("iterator", c.cl_fields)))): gen_int_iter(apply_params(c.cl_params, pl, pt), get_next_array_element, get_array_length);
									case (_, TAbstract({ a_impl = Some(c) } = a, tl)): try {
									...
									case (_, TInst({ cl_kind = KGenericInstance({ cl_path = (::(haxe, ::(ds, [])), GenericStack) }, ::(t, [])) } = c, [])): var tcell = try {
									...
									lblock(::(mk(TVar(cell, Some(mk(mk_field(e1, "head"), tcell, p))), t_void, p), ::(mk(TWhile(mk(TBinop(OpNotEq, cexpr, mk(TConst(TNull), tcell, p)), ctx.t.tbool, p), block, NormalWhile), t_void, p), [])));
									case _: None;
								};
								};
									...
									case TBlock(el): { (e2) with eexpr = TBlock(::(enext, el)) };
									...
									switch (e.eexpr) {
									case TConst(_) | TLocal(_) | TArray(_) | TField(_) | TEnumParameter(_) | TParenthesis(_) | TMeta(_) | TCall(_) | TNew(_) | TTypeExpr(_) | TObjectDecl(_) | TArrayDecl(_): False;
									...
									case _: com.error(^("On static platforms, null can't be used as basic type ", s_type(print_context([]), e.etype)), e.epos);
					  };
					  };
						  loop(e.etype);
					  } else {
						  ...
						  case TBlock(_): f;
						  case _: { (f) with tf_expr = block(f.tf_expr) };
					  };
						  ...
						  case []: e;
						  case ::(ec, l): switch (List.filter(function e: switch (e.eexpr) {
						  case TConst(_) | TBlock([]) | TObjectDecl([]): False;
						  case _: True;
					  }, l)) {
						  case []: ec;
						  case l: { (e) with eexpr = TBlock(List.rev(::(ec, l))) };
					  };
					  };
						  case TParenthesis(ec): { (ec) with epos = e.epos };
						  case TTry(e, []): e;
						  case _: e;
					  };
					  };
						  ...
						  case OpEq: { (e) with eexpr = TConst(TBool(True)) };
						  case OpNotEq: { (e) with eexpr = TConst(TBool(False)) };
						  case _: e;
					  };
						  case (TFunction(_), TConst(TNull)): switch (op) {
						  case OpEq: { (e) with eexpr = TConst(TBool(False)) };
						  case OpNotEq: { (e) with eexpr = TConst(TBool(True)) };
						  case _: e;
					  };
						  ...
						  case (TField(_, FEnum(e1, f1)), TField(_, FEnum(e2, f2))) if(==(e1, e2)): switch (op) {
						  case OpEq: { (e) with eexpr = TConst(TBool(==(f1, f2))) };
						  case OpNotEq: { (e) with eexpr = TConst(TBool(!=(f1, f2))) };
						  case _: e;
					  };
						  case (_, TCall({ eexpr = TField(_, FEnum(_)) }, _)) | (TCall({ eexpr = TField(_, FEnum(_)) }, _), _): switch (op) {
						  case OpAssign: e;
						  ...
						  case (Not, TConst(TBool(f)) | TParenthesis({ eexpr = TConst(TBool(f)) })): { (e) with eexpr = TConst(TBool(!(f))) };
						  case (Neg, TConst(TInt(i))): { (e) with eexpr = TConst(TInt(Int32.neg(i))) };
						  case (NegBits, TConst(TInt(i))): { (e) with eexpr = TConst(TInt(Int32.lognot(i))) };
						  ...
						  case TIf({ eexpr = TConst(TBool(t)) }, e1, e2): if (t) {
						  e1;
					  } else {
						  switch (e2) {
						  case None: { (e) with eexpr = TBlock([]) };
						  case Some(e): e;
					  };
					  };
						  case TWhile({ eexpr = TConst(TBool(False)) }, sub, flag): switch (flag) {
						  case NormalWhile: { (e) with eexpr = TBlock([]) };
						  case DoWhile: e;
					  };
						  case TBinop(op, e1, e2): optimize_binop(e, op, e1, e2);
						  case TUnop(op, flag, esub): optimize_unop(e, op, flag, esub);
						  case TCall({ eexpr = TField({ eexpr = TTypeExpr(TClassDecl(c)) }, field) }, params): switch (api_inline(ctx, c, field_name(field), params, e.epos)) {
						  ...
						  case TSwitch(e1, ::((::({ eexpr = TConst(TBool(True)) }, []), { eexpr = TConst(TBool(True)) }), []), Some({ eexpr = TConst(TBool(False)) })): e1;
						  ...
						  case TBinop(OpAdd | OpSub | OpMult | OpDiv | OpMod = op, e1, e2): switch ((new Tuple(make_constant_expression(ctx, e1), make_constant_expression(ctx, e2)))) {
						  case (Some({ eexpr = TConst(TString(s1)) }), Some({ eexpr = TConst(TString(s2)) })) if(concat_strings): Some(mk(TConst(TString(^(s1, s2))), ctx.com.basic.tstring, punion(e1.epos, e2.epos)));
						  ...
						  case TCall({ etype = TFun(_, ret); eexpr = TField(_, FStatic(c, cf)) }, el): try {
						  ...
						  switch (e.eexpr) {
						  case TNew({ cl_constructor = Some({ cf_kind = Method(MethInline); cf_expr = Some({ eexpr = TFunction(f) }) } = cst) } = c, tl, pl): IKCtor(f, cst, c, tl, pl, []);
						  ...
						  case IKCtor(f, cst, c, tl, pl, el_init) if(type_iseq(v.v_type, n.etype)): switch (try {
						  ...
						  List.iter(function cf: switch ((new Tuple(cf.cf_kind, cf.cf_expr))) {
						  ...
						  case TBinop(OpAssign, { eexpr = TField({ eexpr = TLocal(vv) }, FInstance(_, _, cf)); etype = t }, e) if(==(v, vv)): assigns.val = ::((new Tuple(cf.cf_name, e, t)), assigns.val);
						  case _: raise(Exit);
					  };
					  };
						  try {
						  get_assigns(ecst);
						  vars.val = PMap.add(v.v_id, (new Tuple(v, el_init, List.rev(assigns.val), ||(c.cl_extern, Meta.has(Meta.Extern, cst.cf_meta)), n.epos)), vars.val);
						  v.v_id = ~-(v.v_id);
						  find_locals(ecst);
					  } catch(e:Exit) {
						  [];
						  ...
						  case IKArray(el, t): vars.val = PMap.add(v.v_id, (new Tuple(v, [], ExtList.List.mapi(function i: function e: (new Tuple(string_of_int(i), e, t)), el), False, n.epos)), vars.val);
						  v.v_id = ~-(v.v_id);
						  case IKStructure(fl): vars.val = PMap.add(v.v_id, (new Tuple(v, [], List.map(function (s, e): (new Tuple(s, e, e.etype)), fl), False, n.epos)), vars.val);
						  v.v_id = ~-(v.v_id);
						  case _: [];
					  };
						  case _: [];
					  };
						  case TField(e1, FInstance(_, _, { cf_kind = Var(_); cf_name = s }) | FAnon({ cf_kind = Var(_); cf_name = s })): switch (skip_to_var(e1)) {
						  case None: find_locals(e1);
						  case Some(_): [];
					  };
						  case TArray(e1, { eexpr = TConst(TInt(i)) }): switch (skip_to_var(e1)) {
						  case None: find_locals(e1);
						  ...
						  case TArray({ eexpr = TLocal(v) }, { eexpr = TConst(TInt(i)) }) if(<(v.v_id, 0)): check_field(v, Int32.to_string(i), e2, e2.etype);
						  case TField({ eexpr = TLocal(v) }, FInstance(_, _, { cf_kind = Var(_); cf_name = s }) | FAnon({ cf_kind = Var(_); cf_name = s })) if(<(v.v_id, 0)): check_field(v, s, e2, e2.etype);
						  ...
						  decl(n, None, Some(EBlock(::((new Tuple(EVars(::((new Tuple("$tmp", None, None)), [])), p)), ::((new Tuple(EFor((new Tuple(EIn(id, it), p)), (new Tuple(EBinop(OpAssign, etmp, (new Tuple(EConst(Ident(n)), p))), p))), p)), ::(etmp, [])))), p));
						  var efor = loop(efor);
						  old([]);
						  ...
						  var cases = List.map(function (el, eg, eo): switch (eo) {
						  case None: (new Tuple(el, eg, eo));
						  case Some(e): var el = List.map(loop, el);
						  var old = save([]);
						  List.iter(function e: switch (fst(e)) {
						  ...
						  case EFunction(_, f): Ast.map_expr(subst_locals({ () with r = PMap.foldi(function n: function i: function acc: if (List.exists(function (a, _, _, _): =(a, n), f.f_args)) {
						  acc;
					  } else {
						  PMap.add(n, i, acc);
					  }, locals.r, PMap.empty) }), e);
						  ...
						  var e = (new Tuple(EBlock(::((new Tuple(EVars(List.rev(tmp_locals.val)), p)), ::((new Tuple(EDisplay(e, call), p)), []))), p));
						  raise(Return(e));
					  } catch(e:Exit) {
						  map(e);
					  };
